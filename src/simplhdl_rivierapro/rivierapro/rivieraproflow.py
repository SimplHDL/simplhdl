...
from ..resources.templates import rivierapro as rivieraprotemplates
...
class RivieraProFlow(SimulationFlow):
    ...
    def parse_args(self, subparsers) -> None:
        ...
        parser.add_argument("--gui", action="store_true", help="Open project in Riviera PRO GUI")
        parser.add_argument(
            "--vsim-args",
            default="",
            action="store",
    ...
    def is_tool_setup(self) -> None:
        if (
            shutil.which("vlog") is None
            or shutil.which("vsim") is None
            or shutil.which("vcom") is None
            or shutil.which("vlib") is None
            or shutil.which("vmap") is None
        ):
